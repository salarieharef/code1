      ...
      <Link href={`/profile/${data?.id}`}>
        <CardContent className='flex h-full flex-col items-center justify-center text-nowrap p-4 '>
          <Avatar className='size-20 border-b-4 border-b-amber-500 sm:size-36'>
            ...
            <AvatarFallback>
              ...
              <div className='absolute bottom-0 mb-2'>
                {first_name ? first_name.charAt(0).toUpperCase() : null}‌.
                {last_name ? last_name.charAt(0).toUpperCase() : null}
              </div>
            </AvatarFallback>
          </Avatar>
          ...
          <div className='mt-2 flex flex-wrap justify-around gap-x-4 gap-y-2'>
            ...
            <span className='text-xs text-primary'>{likes || 0} پسندیدن</span>
            <span className='text-xs text-primary'>{views || 0} بازدید</span>
          </div>
        </CardContent>
      </Link>
