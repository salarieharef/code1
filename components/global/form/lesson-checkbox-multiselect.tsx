...
export default function LessonCheckboxMultiselect({
  valueLoading,
  label,
  placeholder = "عنوان جلسه خود را انتخاب کنید...",
  name = "lesson",
  required,
  disabled,
  className,
  ...
  const {
    ...
    (index) =>
      courseValue
        ? [
            routes.courseRoutes.courseLessons(courseValue, index + 1),
            debouncedSearch,
          ]
        : null,
    ([url]) =>
  ...
  const allLesson = [allOption, ...lessons];

  const Items = isAllItem && lessons.length >= 2 ? allLesson : [...lessons];
        ...
        <FormItem className='text-right'>
          ...
          <FormControl>
            <MultiSelectCheckBox
              ...
              disabled={!courseValue || valueLoading}
              loading={lessonIsLoading}
              isValidating={lessonIsValidating}
              setSize={lessonSetSize}
              size={lessonSize}
              page_number={get(lessons, "[0].page_number", 0)}
              onSearch={setSearch}
              defaultValue={
                field.value?.length
