...
// Type imports
import { VerticalStepProps } from "./types.d";
...
// Constant imports
import { verticalStepVariants } from "./constant";
    ...
    <>
      {React.Children.map(children, (child: any, i) => {
        const isCompletedStep =
          ...
          child?.props?.id < activeStep;
        const isLastStep =
          child?.props?.details?.id === stepCount - 1 ||
          child?.props?.id === stepCount - 1;
        const isCurrentStep =
          child?.props?.details?.id === activeStep ||
          child?.props?.id === activeStep;
  ...
  (props, ref) => {
    ...
    return (
      <div
        ...
        onClick={() =>
          onClickStep?.(id || index || 0, setStep) ||
          onClickStepGeneral?.(id || index || 0, setStep)
        }
      >
        <div
          data-vertical={true}
...
export { VerticalStep, VerticalContent };
