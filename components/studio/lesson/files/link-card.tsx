...
export function LinkCard({ removeFile, mutate, data }: any) {
  ...
  const AddLink: SubmitHandler<Inputs> = async (data: any) => {
    if (!data?.id) {
      try {
        ...
        //   routes.lessonRoutes.addLink(params.lessonId),
        //   {
        //     type: data.type,
        //     title: data.title,
        //     link: data.link,
        ...
        const res = await nextFetcher({
          url: routes.lessonRoutes.addLink(params.lessonId),
          method: "POST",
          body: {
            type: data.type,
            title: data.title,
  ...
  const removeRemote = async () => {
    try {
      setDeleting(true);
      // const res = await postFetcher(
      // 	routes.lessonRoutes.deleteLink(params?.lessonId, data?.id)
      // );
      const res = await nextFetcher({
        url: routes.lessonRoutes.deleteLink(params?.lessonId, data?.id),
        method: "POST",
        useToken: true,
      });
