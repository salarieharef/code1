...
// Config imports
import { studioFieldMenu } from "@/constant/studio/menus";
...
export default function FieldSidebar({ className }: any) {
  const { data: session }: any = useSession();
  const params = useParams();
  const fieldId = params?.fieldId;
  const pathname = usePathname();
  const menuItems = studioFieldMenu(fieldId);

  // const { data: classDetails } = useSWR(
  //   routes.courseRoutes.detail(fieldId),
  //   fetcher,
  //   {
