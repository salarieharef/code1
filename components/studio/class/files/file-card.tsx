...
export function FileCard({ removeFile, mutate, data }: any) {
  ...
  const AddFile: SubmitHandler<Inputs> = async (data: any) => {
    if (!data?.id) {
      try {
        ...
        //   routes.courseRoutes.addContent(params.classId),
        //   dataFormData,
        //   {},
        //   (progressEvent: any) => {
        ...
        const res = await nextFetcher({
          url: routes.courseRoutes.addContent(params.classId),
          method: "POST",
          body: dataFormData,
          useToken: true,
          options: {
  ...
  const removeRemote = async () => {
    try {
      setDeleting(true);
      // const res = await postFetcher(
      //   routes.courseRoutes.deleteContent(params?.classId, data?.id)
      // );
      const res = await nextFetcher({
        url: routes.courseRoutes.deleteContent(params?.classId, data?.id),
        method: "POST",
        useToken: true,
      });
                      ...
                      <>
                        {!data?.id ? (
                          <ReusableFileDropzone
                            name='file'
                            placeholder='فایل مورد نظر خود را برای آپلود اینجا بکشید و رها کنید.'
                            accept={
