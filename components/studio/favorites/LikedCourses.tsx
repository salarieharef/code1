...
export default function LikedCourses() {
  ...
  // error,
  // isLoading,
  // } = useSWR(routes.userRoutes.likedCourses({ page: page }), fetcher, {
  ...
  const {
    ...
    routes.userRoutes.likedCourses({ page: page }),
    (url) =>
      nextFetcher({
        url,
        method: "GET",
  ...
  return (
    <>
      <div className='space-between mt-6 grid gap-x-8 gap-y-4 sm:grid-cols-2 md:grid-cols-2'>
        {isLoading ? (
